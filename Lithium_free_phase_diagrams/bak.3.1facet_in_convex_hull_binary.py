...
chemsys_list = pd.read_csv('tables/K/chemsys_binary.csv').chemsys.to_list()
...
with MPRester(api_key='25wZTKoyHkvhXFfO') as mpr:
    for chemsys in tqdm(chemsys_list, total=len(chemsys_list)):
        ...
        tieline_entries_dict = tieline_phases(phaseDiagram, key_element='K')
...
tieline_dataframe.to_csv('K_tieline_binary.csv', index=False)

tieline_dataframe.drop_duplicates('entry_id').to_csv('K_tieline_ids_binary.csv', index=False)
# tieline_unique_id_dataframe.to_csv('K_tieline_ids.csv', index=False)

tieline_dataframe.drop_duplicates('pretty_formula').to_csv('K_tieline_names_binary.csv', index=False)
